const translations = {
    en: {
        //header
        home: 'Home',
        service: 'Service',
        serviceUs:'Our Service',
        about: 'About US',
        contact: 'Contact Us',
        FreeLegalAdvice: 'Free Legal Advice',

        //about us
        aboutUsB: "Saleh Al Saleh Advocates & Legal Consultants provide a wide range of legal services through legal expertise and in professional ways, ensuring transactional satisfaction from any legal problems facing our valued clients. The Office deals in all kinds of civil, commercial and criminal cases and provides legal services in various fields and at all levels. Advocate\ Saleh Al Saleh is one of the distinguished lawyers in the UAE and has more than 21 years of experience in the field of law. The office has a good reputation. Advocate\ Saleh Al Saleh is interested in appointing the best lawyers and advisors with experience in the field of law for many years.",
        //Contact Us
        CallUs: "Call Us",
        address:"Address",
        Name:"Name",
        Send:"Send Message",
        enterName:"Enter your name",
        enterEmail:"Enter your email",
        enterMessage:"Enter your message",
        Message: "Message",
        Email:"Email",
        Location:"Location",
        address1:"UAE-Dubai,Deira,Al khabaisi,Behind NISSAN Showrooms,Business Point Building,Office No.403",

        // Service
        LegalConsultations: "Legal Consultations",
        legalDocuments: "Preparation and drafting of legal documents",
        LegalProcedures: "Legal procedures, registration legal credits",
        LegalRepresentation: "Legal Representation",
        ResearchServices: "Research services",
        PenalCases: "Penal cases",
        CriminalCases: "Criminal Cases",
        CybercrimeLawsuit: "Cybercrime Lawsuit",
        LaborLawsuit: "Labor Lawsuit",
        RentDisputes: "Rent Disputes",
        FamilyIssuesAndConditions: "Family issues and conditions",
        Insurance: "Insurance",
        MaritimeDisputes: "Maritime disputes",
        BankingAndFinance: "Banking and Finance",
        TradingCompanies: "Trading companies",
        LitigationAndDisputeResolution: "Litigation And Dispute Resolution",
        PrecautionaryMeasures: "Precautionary measures",
        ExecutiveActions: "Executive Actions",
        RealEstateDisputes: "Real estate disputes",
        DebtCollection: "Debt Collection",

        // Legal Consultations
        Saleh_Al_Saleh_Legal_Consultations: "Saleh Al Saleh Advocates and Legal Consultants",
        SalehAlSalehLegalConsultationsB1: "Provides legal advice through a distinguished selection of legal advisors. It is characterized by an office that keeps pace with today's development s and techniques. Inthis accelerating era, every human being needs to know his or her rights, duties andlegal attitude towards an order, or a project submitted to him, with great speed andprecision. We therefore took care of this, and we did not confine ourselves totraditional ways of communicating betweenthe Office and its clients eager to take legal advice on a subject. Traditionally, anyclient with legal advice must go to thelaw firm specializing in it. At Saleh Al Saleh Advocates & Legal Consultants, it isdifferent where you can send legal advice to us through email,Explaining all the details relating to them, one of the Bureau's specialized adviserswill consult them and give their legal opinion,supported by the texts of the law, and then send them to you by e-mail or fax, takinginto account the confidentiality of all the data and informationcontained in the consultation.We therefore took care of this, and we did not confineourselves to traditional ways of communicating betweenthe Office and its clients eager to take legal advice on a subject. Traditionally, anyclient with legal advice must go to thelaw firm specializing in it. At Saleh Al Saleh Advocates & Legal Consultants, it isdifferent where you can send legal advice to us through email,Explaining all the details relating to them, one of the Bureau's specialized adviserswill consult them and give their legal opinion,supported by the texts of the law, and then send them to you by e-mail or fax, takinginto account the confidentiality of all the data and informationcontained in the consultation",
        Saleh_Al_Saleh_Legal_ConsultationsB2: "As legal consulting plays an important and effective role in guiding and advising clients before commencing legal proceedings, the office has been interested in providing the best services to clients through the legal consulting service provided by aspecialized team Who provides legal advice through qualified consultants, withdistinguished and specialized scientific and expert ammunition and through the latesttechnology whether on phone, internet, email, fax and SMS. The Legal Consulting Servicerequires the prompt provision of advice to enable the respondent to make decisionsquickly and accurately.",
        Saleh_Al_Saleh_Legal_ConsultationsB3: "provides consultations in Arabic and English in all legal aspects that meet individuals and companies their daily lives or in investments and business matters by providing legal advice and advice on the appropriate action to protect the client's interests ineach problem to ensure that the client has an understanding of his problem and itsconsequences and the actions to be taken to resolve the problem in order to avoidserious consequences.",
        Saleh_Al_Saleh_Legal_ConsultationsB4: "provides legal advice whether by arranging a meeting date with the competent lawyer and legal advisor responsible for the office or through electronic correspondence or telephone confirmation. The Office provides legal advice in writing and covered by legalrights of law, court practices and interpretations of jurists to indicate the sound bondto assure the client of the validity of the legal advice and its support.",
        Saleh_Al_Saleh_Legal_ConsultationsB5: "is linked to several annual consultancy contracts with commercial companies and financial institutions that wish to receive continuous services throughout the year whether with a lump-sum contract with a fixed or indefinite number of hours or undercontracts according to the working hours of consultants, legal researchers and lawyersto ensure the diversity of choices for clients.",
        Saleh_Al_Saleh_Legal_ConsultationsB6: "is a source of trust because the legal consultations issued by Al Saleh Law Firm are always accompanied by evidence, source and comprehensive statement adequate to first understand the client in order to ensure that he is assured of the soundness of the grounds on which the legal consultation was issued.",
        Lookingforlegaladvice: "Looking for legal advice? Contact us, and we'll start productive cooperation.",

        // Preparation and drafting of legal documents
        Preparation_legal_documentsB: "is the most famous office in the preparation and legal drafting in the UAE where we have a specialized team of legal advisers and researchers with expertise in the preparation and drafting of legal editions and documents of various forms and types The Office provides preparation and drafting services in Arabic and English. This includes all the implementation of companies and institutions with limited responsibility, private and public contributions, entrepreneurship and individuals as examples of such services towards the formulation and liberalization of",
        Preparation_legal_documents1: "Memorandum of Association and Their Annexes",
        Preparation_legal_documents2: "Mergers and acquisitions contract and agreement",
        Preparation_legal_documents3: "Commercial agency contracts",
        Preparation_legal_documents4: "Arbitration agreements",
        Preparation_legal_documents5: "Trade Sponsorship Contracts and Agreements",
        Preparation_legal_documents6: "Endowment and Will Contracts",
        Preparation_legal_documents7: "Mortgage contract",
        Preparation_legal_documents8: "Concession contracts",
        Preparation_legal_documents9: "Employment contracts",
        Preparation_legal_documents10: "Reconciliation agreements",
        Preparation_legal_documents11: "Marketing contracts",
        Preparation_legal_documents12: "Influencers and Influences Management Agreement",
        Preparation_legal_documents13: "Memorandums of Understanding",
        Preparation_legal_documents14: "Liquidation Minutes",
        Preparation_legal_documents15: "Publicity Minutes",
        Preparation_legal_documents16: "Affidavits Oaths",
        Preparation_legal_documents17: "Authorization Letter",
        Preparation_legal_documents18: "Internal Disciplinary Regulations",
        Preparation_legal_documents19: "Legal opinion memorandum",
        Preparation_legal_documents20: "Regulatory terms and conditions",
        Preparation_legal_documents21: "Drafting of opinion memorandum and legal advice",
        Preparation_legal_documents22: "Legal Correspondence",
        Preparation_legal_documents23: "Formulation of complaints and suggestions",
        Preparation_legal_documents24: "Writing legal correspondence",
        Preparation_legal_documents25: "Partnership contracts and agreements",
        Preparation_legal_documents26: "Shareholders' contract",
        Preparation_legal_documents27: "Confidentiality of information Agreements",
        Preparation_legal_documents28: "Trademark contracts",
        Preparation_legal_documents29: "Real estate rights contracts",
        Preparation_legal_documents30: "Loan contracts",
        Preparation_legal_documents31: "Waiver Contracts",
        Preparation_legal_documents32: "Lease contracts",
        Preparation_legal_documents33: "Settlement agreements",
        Preparation_legal_documents34: "Contracting contracts",
        Preparation_legal_documents35: "Logistics Services contracts",
        Preparation_legal_documents36: "Terms and Conditions",
        Preparation_legal_documents37: "Minutes of ordinary and extraordinary public associations of all companies",
        Preparation_legal_documents38: "Bankruptcy Minutes",
        Preparation_legal_documents39: "Meetings Minutes",
        Preparation_legal_documents40: "Private & General Power of Attorney",
        Preparation_legal_documents41: "Organizational regulations and corporate regulations",
        Preparation_legal_documents42: "Various legal notes",
        Preparation_legal_documents43: "Audit  legal documents",
        Preparation_legal_documents44: "Legal review and analysis of contracts and conventions",
        Preparation_legal_documents45: "Legal warnings",
        Preparation_legal_documents46: "Drafting legal warnings",
        Preparation_legal_documents47: "Writing various legal note",

        //Legal procedures, registration and legal credits
        LegalProcedures: "Legal procedures, registration and legal credits",
        LegalProceduresB: "offers legal procedures, registration and legal credits in the United Arab Emirates where we have a specialized team of lawyers and legal assistants with expertise in conducting procedures in various fields and various governmental and private works including all legal enforcement. Examples of such services are:",
        LegalProceduresB1: "Registration of companies and institutions with limited liability",
        LegalProceduresB2: "Corporate Registration and entrepreneurship",
        LegalProceduresB3: "Trademarks",
        LegalProceduresB4: "Insolvency",
        LegalProceduresB5: "Registration of private and public joint stock companies",
        LegalProceduresB6: "Intellectual property registration",
        LegalProceduresB7: "Liquidation",
        LegalProceduresB8: "Bankruptcy",
        LegalProceduresB9: "- Change: towards change of trade name, change of activity, change of management items Change of capital, change of company's duration or other terms of contract change one or more of the objectives of the Service's partnership with partners, Significant change in the organizational structure of the Service or partners affecting the management and status of the partnership Change in the structure of common services between the Service and partners, change one or more of the objectives of the Service's partnership with partners s organizational structure or partners affecting the management and status of the partnership concluded, Review the current partnership contract and submit a report to senior management indicating the scope of the partnership's change to negotiate with partners the objectives or services to be changed or added to the partnership contract, Adjustment and change according to lower quality of services provided through partners to amend or amend government legislation and laws in contravention of pre-concluded partnership agreements; Prepare the amended partnership contract in coordination with the Department of Legal Affairs, sign the amended partnership contract and cover the event informatively if the amendment is material and affects the clients; Establishment of a joint group to implement the amended provisions of the Convention, and of a joint group to implement the amended provisions of the Convention.",
        LegalProceduresB10: "Inventory of legacies",
        LegalProceduresB11: "Proof of mental ability and eligibility",
        LegalProceduresB12: "The guardian's personal information (supported by his passport)",
        LegalProceduresB13: "Whether another will relates to the testator's property and place of registration and if she wishes to cancel it",
        LegalProceduresB14: "Bank account details",
        LegalProceduresB15: "Proof of religion",
        LegalProceduresB16: "The elements needed to write the will",
        LegalProceduresB17: "Family tree (family members and their guardian's dependents)",
        LegalProceduresB18: "Property details are supported by copies of title deeds",
        LegalProceduresB19: "Details of any other property",
        LegalProceduresB20: "Consultants provides certification services from relevant authorities for foreign documents issued abroad as well as certification services for documents required by foreign agencies abroad by certifying the certified lawyer's stamp, including: Certification of the copy of travel passports and the copy of the invoices of services issued by local authorities and certificates of confirmation of addresses for individuals and companies. Certification services include all other papers such as commercial licenses, incorporation contracts, agreements, declarations, etc., which may require the establishment of offshore companies, opening of bank accounts abroad or applications for immigration and travel.",
        LegalProceduresB21: "provides its services by viewing the authenticity of the documents and investigating the authenticity of the information contained in those statements and their source thereafter the certification and sealing of the relevant lawyer and the client receives a certified copy of the documents.",
        LegalProceduresB22: "Certification services issued by Saleh Al Saleh Advocates & Legal Consultants include confirmation of the authenticity of a person's signature in front of the lawyer through the presence of the person to be signed on the contract and after consulting his identity and passport. The contract is signed in front of the competent lawyer who signs the validity of the said person's signature in connection with the requirements of the sale of real estate in the United States Kingdom and foreign countries.",
        LegalProceduresB23: "The entities receiving the certificates can review Saleh Al Saleh Law and Legal Advice to verify the authenticity of the lawyer's signature and the office will confirm the authenticity of the documents issued to the said authorities.",

        //Legal Representation
        LegalRepresentation: "Legal Representation",
        LegalRepresentationB: "Customers who perform business in the UAE or with Emirati companies may find themselves liable to litigation in the courts. Our legal experts have considerable experience in commercial litigation in UAE courts, representing plaintiffs and defendants. We understand the complex practices of the region's litigation process and can provide you with representation in difficult aggressive rivalries.",
        LegalRepresentationB1: "has the experience and know-how in recording and following up all types of cases, lawsuits, and pleadings, and practical and scientific know-how in various types of civil, commercial, administrative, and enforcement procedures, labor and legal cases, and criminal cases in all types of courts of all degrees, primary - appeal and the Federal Supreme Court (cassation) at the level the state",
        LegalRepresentationB2: "This includes:- Representing clients and appearing before all types and levels of courts, including the Court of First Instance, the Court of Appeal, the Court of Cassation, Enforcement, Examination or Interrogation, the Clerk of Justice, the Attorney General, other judicial bodies, police services, all bodies, services, references and other official ministries within and outside the United Arab Emirates.",
        LegalRepresentationB3: "Attending investigations",
        LegalRepresentationB4: "Filing, pleading, following up and executing claims...etc",
        LegalRepresentationB5: "Submit a notification",

        //Research services
        Researchservices: "Research services",
        ResearchservicesB: "Saleh Al Saleh Advocates and Legal Consultants provides legal research services through the preparation of specialized legal research and studies in various branches of laws and regulations through the Department of Scientific Research, Legitimacy and Jurisprudence, which is undoubtedly one of the most important references used in the fields of law and consulting . Saleh Al Saleh Advocates and Legal Consultants also offers legal training services through the holding of training legal courses for employees of private and public companies and institutions according to high scientific specifications that accommodate all legal disciplines, using the highest legal competencies.",
        //Penal cases
        PenalCases: "Penal cases",
        PenalCasesB: "Saleh Al Saleh Advocates and Legal Consultants works to represent clients in criminal proceedings, whether irregularities, misdemeanors or felonies before the courts of first instance, appeals and cassation, according to the types of such criminal courts between passage, municipality, public prosecution and state security.",
        //Criminal Cases
        CriminalCases: "Criminal Cases",
        CriminalCasesB: "Saleh Al Saleh Advocates and Legal Consultants works to represent clients in numerous criminal proceedings, whether offences, misdemeanors or felonies before the courts of first instance, appeals and cassation of different kinds of criminal courts between the traffic courts, municipal courts, public prosecution courts, state security courts and military courts, as well as infringements of laws on different kinds of accusations against clients, with a view to achieving innocence in such cases and favour .",
        CriminalCasesB1: "Saleh Al Saleh Advocates and Legal Consultants represents clients in criminal proceedings and varies between serious traffic offences and municipal claims about violated construction, failure to follow safety standards and restaurant infractions, as well as representation of clients before immigration courts, stay in irregular residency and flight reports, as well as public prosecution cases such as insult, slander, libel, beatings, assault, manslaughter, infringement of the court, trafficking and trafficking of property.",
        CriminalCasesB2: "Saleh Al Saleh Advocates and Legal Consultants also represents clients in infractions and penalties issued on the basis of special laws such as electronic offences for the commission of crimes using information technology means such as text messages, chat programs and hack programs, as well as infringement of intellectual property protection laws by placing protected materials on the Internet, their personal use or trafficking, as well as violations of consumer rights protection law and all laws related to infringements.",
        CriminalCasesB3: "Saleh Al Saleh Advocates and Legal Consultants works to achieve the innocence of the clients by presenting the defence before the courts, attending, representing the client and presenting the arguments that ensure the presentation of the defendants' and clients' defence and ensuring that the judges understand the dispute through the language and fluency of the lawyer present in the dispute so as to ensure the best results and protect the interests of the clients",
        //Cybercrime Lawsuit
        CybercrimeLawsuit: "Cybercrime Lawsuit",
        CybercrimeLawsuitB: "The technology's global trade. The latest IT laws are adopted to enable you to legally excel in this field by keeping pace with transformations and under Federal Law No. 1.",
        //Labor Lawsuit
        LaborLawsuit: "Labor Lawsuit",
        LaborLawsuitB: "Saleh Al Saleh Advocates and Legal Consultants works to represent many employees and employers from enterprises, companies and individuals in cases filed before the labour courts to claim and settle accounts due between the parties. Saleh Al Saleh Advocates and Legal Consultants represents many employees and employers from enterprises, companies and individuals in the lawsuits filed before the labour courts in order to claim and settle the accounts due between the parties. Saleh Al Saleh Legal Consultants represents employees or employers in those cases before the courts of first instance, appeals, cassation and experience assigned to the accounting and liquidation of workers' entitlements for both fixed and indefinite judicial contracts against private or governmental or semi-governmental companies before both human and private courts.",
        LaborLawsuitB1: "Saleh Al Saleh Advocates and Legal Consultants represents employees in claiming their workers' benefits by claiming late salaries, arbitrary dismissal, accrued leave balance, end of service, arbitrary dismissal, overtime, working allowance during holidays and public holidays, claiming periodic equivalents, commissions and ratios, as well as seeking compensation for direct damages in the event of a breach of their obligations by the employer Delivery of delivered materials to employees, unlawful competition, breach of confidentiality and damages to the employer for the staff member's gross error and exemption of the employer from claims for overtime for staff in senior management positions.",
        LaborLawsuitB2: "Saleh Al Saleh Advocates and Legal Consultants has practical experiences in achieving the best results in presenting the facts and legal principles that guarantee clients the desired result in cases of arbitrary dismissal and cases whether by forcing the worker to leave work or submit his resignation or false reports of escape, arbitrary suspensions and extensions of separation and employment under assignment contracts to ensure that the client has practical experiences during his representation in the required disputes.",
        //Rent Disputes
        RentDisputes: "Rent Disputes",
        RentDisputesB: "Undoubtedly, the UAE is one of the countries that relies heavily on real estate investment. A close follow-up of the UAE's real estate market and more recently.Saleh Al Saleh Advocates and Legal Consultants prepares, lists and follows up on all rental advocacy - eviction - compensation - reimbursement of rental value - assignment of expertise to evaluate the value of the rent of the property and any damages if any. and before the Urgent Matters Judge if necessary in some and all cases: Primary / Appeal / Cassation",
        //Family issues and conditions
        FamilyIssues: "Family issues and conditions",
        FamilyIssuesB: "Saleh Al Saleh Advocates and Legal Consultants works to represent many individuals in family disputes whether Muslim or non-Muslim. In the presence of defence and proceedings before the Family Steering Committee of family courts within the UAE, litigation varies from divorce according to state law or divorce according to the laws of foreign countries or cases of claiming alimony and custody in accordance with state law.",
        FamilyIssuesB1: "Family disputes are a wide and intertwined area of proceedings and proceedings before the courts. They have numerous temporary and expeditious procedures to guarantee the rights of the parties, including temporary maintenance claims for the wife whose husband refrains from spending on them, including children's alimony, the handover of children to the disputed father or mother, the temporary vision, counselling and counselling of clients on litigation proceedings during the hearings, where sometimes the personal attendance of the client, the children, as well as well as the sentencing procedures of the children, are not a large part of the children .",
        FamilyIssuesB2: "Saleh Al Saleh Advocates and Legal Consultants represents disputants in family lawsuits through its years of experience in achieving the best results for the disputants. Saleh Al Saleh Law Firm and Legal Consultations is interested in achieving those results quickly, especially in cases of the presence of children for the purpose of protecting them as they are the weak party in the subject of the dispute.",
        //Insurance
        Insurance: "Insurance",
        InsuranceB: "The commercial and industrial renaissance led to the growth and prosperity of the insurance sector and increased the insurance awareness of all. Due to our theoretical and practical experiences in this field, Saleh Al Saleh Advocates & Legal Consultants provides its legal services to the insurance sector of the United Arab Emirates, both for companies and individuals.",
        InsuranceB1: "- Provide legal opinion and advice regarding insurance coverage for individuals and companies of various kinds such as fire, loss of property, car accidents, professional negligence, personal injury.",
        InsuranceB2: "- Representing clients in all issues related to insurance disputes",
        InsuranceB3: "- Resolve insurance disputes and negotiate payment with insurance companies",
        //Maritime disputes
        MaritimeDisputes: "Maritime disputes",
        MaritimeDisputesB: "Saleh Al Saleh Advocates and Legal Consultants is part of a few legal offices with knowledge and research in the field of maritime law. Saleh Al Saleh Law and Legal Consulting members are keen to provide the necessary research and studies in this regard.",
        //Banking and Finance
        BankingFinance: "Banking and Finance",
        BankingFinanceB: "Banking and financial institutions in the country are one of the most important pillars of the national economy. They are therefore highly interested in the economy. Saleh Al-Saleh Law Firm and Legal Consultants recognized this importance and worked to represent many banking institutions and was interested in the work of these institutions.",
        //Trading companies
        TradingCompanies: "Trading companies",
        TradingCompaniesB: "Al Saleh Al Saleh Lawyers and Legal Consultants Team is experienced and continuously following up on all laws that determine the shape of the company and the procedures for establishing each of them. Our lawyers and advisors can provide advice and guidance to all investors .",
        //Litigation and Dispute Resolution
        LitigationResolution: "Litigation and Dispute Resolution",
        LitigationResolutionB: "Litigation is the most widely used means of dispute resolution in the United Arab Emirates and Saleh Al Saleh Advocates & Legal Consultants has an unprecedented team of trusted citizen lawyers supported by a number of legal advisers with knowledge and expertise in many legal aspects.",
        LitigationResolutionB1: "Saleh Al Saleh Advocates and Legal Consultants has a team with unprecedented trust from citizen lawyers supported by a number of legal advisers with knowledge and expertise in many legal aspects: civil, commercial, criminal, labour and corporate disputes. our team represents all clients before various courts, whether in the courts of Primary / Appeal / Cassation in the Seven Emirates.",
        //Precautionary measures
        PrecautionaryMeasures: "Precautionary measures",
        PrecautionaryMeasuresB: "Issuing orders for the confiscation of property, bank accounts and balances by any third party, preventing individuals from travelling, etc., in order to prevent the other party from concealing property or leaving the State to escape.",
        //Executive Actions
        ExecutiveActions: "Executive Actions",
        ExecutiveActionsB: "Legal representation in courts Our client lawyers are represented before judges in courts and in all proceedings, including:",
        ExecutiveActionsB1: "Submission of cases and preparation of warnings, pleadings, appeals, addresses and correspondence with the courts and the other party during all stages of litigation before the court of first instance, appeal and cassation. nforcement of judicial rulings We work through compulsory measures to liquidate the property of the other party and its bank accounts in order to implement judicial rulings.",
        // Real estate disputes
        RealEstateDisputes: "Real estate disputes",
        RealEstateDisputesB: "Real estate disputes are among the most important issues related to real estate and property ownership in the United Arab Emirates. and these disputes vary according to the nature of the real estate relationship, whether it is about the property's ownership, construction or rent. and other real estate disputes, and to be able to resolve and remedy these disputes in a sound manner To avoid making mistakes that may not be commended, you should use the services of our real estate lawyer and use our advanced services to resolve these disputes in a systematic manner and analyse the associated risks. as well as the enjoyment of real estate contract drafting services, and everything related to the property .",
        RealEstateDisputesB1: "Real Estate Dispute Lawyers in favor of The Favor of Law and Legal Consulting help you resolve your real estate disputes with ease and ease, because they are recognized professionals, so that they can handle matters related to your real estate disputes with professionalism, whatever the complexity and size of the case, due to their long and accumulated experience in the fields of real estate and the disputes arising therefrom. Our Office's working group is known for specializing in a number of methods of dispute resolution, including, but not limited to, arbitration, mediation and other alternatives to resolving real estate disputes.",
        RealEstateDisputesB2: "As our team of lawyers specializing in real estate have long and successful skills and experience in this field, your path to litigation, if necessary, should be through them; Because they are the most capable of pursuing these disputes before the courts.",
        //Debt Collection
        DebtCollection: "Debt Collection",
        DebtCollectionB: "Saleh Al Saleh Advocates and Legal Consultants has a team of experts with high competencies and diverse experience in the field of debt collection, settlements and friendly rescheduling with clients in various fields such as engineering, construction and financial institutions.",
        DebtCollectionB1: "Saleh Al Saleh Advocates and Legal Consultants in the field of debt collection in accordance with professional standards, rules and regulations and under a legitimate umbrella compatible with the teachings of Islam. UNODC has the potential to achieve the greatest success in debt recovery by using many solutions that may change depending on the debt and debt situation and nature.",
        DebtCollectionB2: "Saleh Al Saleh Advocates and Legal Consultants has a team of experts with high competencies and varied experience in debt collection, settlements and friendly rescheduling with clients in various fields such as engineering, construction, financial and banking institutions, insurance, companies, etc.",
        DebtCollectionB3: "Saleh Al Saleh Advocates and Legal Consultants follows a specialized debt collection strategy where the team begins - first - to communicate with the debtor and then prepare for a meeting with him in the presence of a legal adviser specializing in debt collection and disputes and has full knowledge of the laws in force in this area, for the purpose of negotiating the settlement of the amount of the debt in the client's interest",
        DebtCollectionB4: "However, this does not prevent Saleh Al Saleh Advocates and Legal Consultants, if required, from taking the necessary legal action against the debtor, by bringing judicial groups against him as required, whether criminal or civil.",


        // body
        Saleh_Al_Saleh: "Saleh Al Saleh Advocates & Legal Consultants",
        body_index: "Saleh Al Saleh Advocates & Legal Consultants is one of the most prominent legal services providers in the UAE through legal expertise and in professional ways, ensuring transactional satisfaction from any legal problems facing our valued clients. The Office deals in all kinds of civil, commercial and criminal cases and provides legal services in various fields and at all levels.",
        OurMission: "Our Mission",
        OurVision: "Our Vision",
        OurMissionBody: "we to provide exceptional legal guidance and support to our clients. We strive to offer personalized solutions, tailored to their unique needs and circumstances.  Our goal is not only to navigate legal complexities effectively but also to empower our clients with knowledge and confidence in their legal matters. With a commitment to integrity, professionalism, and client satisfaction, we aim to be a trusted partner in achieving our clients' legal objectives",
        OurVisionBody: "we to be the foremost destination for legal expertise and guidance, recognized for our unwavering commitment to excellence, innovation, and client satisfaction. We aspire to set new standards in the legal industry by fostering a culture of collaboration, integrity, and forward-thinking. Through our dedication to staying ahead of legal trends and advancements, we aim to empower individuals and businesses alike, ensuring they navigate legal challenges with confidence and clarity.",

        OurServicesT: "Our Services",
        OurServicesB: "We provide a wide range of legal services tailored to meet our clients needs.",
        OurServicesO: "Our goal is to provide reliable and effective legal support, empowering clients to navigate complex legal matters confidently.",

        Company:"Company",
        SocialMedia:"Social Media",
      
        visiontolife: "Reach out and let's bring your vision to life.",
        english: 'English',
        arabic: 'عربي',
        language2: 'Language',

    },

    ar: {
        Company:"عن الشركة",
        SocialMedia:"التواصل الاجتماعي",
        
      
        //header
        home: "الصفحة الرئيسية",
        service: 'خدماتنا',
        serviceUs:'خدماتنا',
        about: 'من نحن',
        contact: 'تواصل معنا',
        FreeLegalAdvice: 'استشارة مجانا',

        //about us
        aboutUsB: "تقدم مكتب صالح الصالح للمحاماة والاستشارات القانونية مجموعة واسعة من الخدمات القانونية من خلال الخبرة القانونية وبطرق محترفة، مضمونة رضا العملاء من أي مشاكل قانونية تواجههم. يتعامل المكتب في جميع أنواع القضايا المدنية والتجارية والجنائية ويقدم خدمات قانونية في مختلف المجالات وعلى جميع المستويات. المحامي صالح الصالح هو أحد المحامين المتميزين في الإمارات العربية المتحدة ولديه أكثر من 21 عامًا من الخبرة في مجال القانون. يتمتع المكتب بسمعة جيدة. يولي المحامي صالح الصالح اهتمامًا خاصًا بتعيين أفضل المحامين والمستشارين ذوي الخبرة في مجال القانون لسنوات عديدة.",
        //Contact Us
        CallUs: "اتصل بنا",
        address:"العنوان",
        enterName:"الرجاء ادخال الاسم",
        enterEmail: "الرجاء ادخال الايميل",
        enterMessage: "الرجاء ادخال نص",
        Name:"الاسم",
        Message: "رسالة",
        Send:"ارسال الرسالة",
        Email:"الايميل",
        Location:"الموقع",
        address1:"الإمارات العربية المتحدة - دبي، ديرة،الخبيصي،بورت سعيد، خلف معارض نيسان، مبنى بزنزبوينت، مكتب رقم 403.",
        // Service
        LegalConsultations: "الاستشارات القانونية",
        legalDocuments: "إعداد وصياغة الوثائق القانونية",
        LegalProcedures: "الإجراءات القانونية، تسجيل الاعتمادات القانونية",
        LegalRepresentation: "التمثيل القانوني",
        ResearchServices: "خدمات البحث",
        PenalCases: "قضايا جنائية",
        CriminalCases: "قضايا جنائية",
        CybercrimeLawsuit: "   الجرائم الإلكترونية",
        LaborLawsuit: "دعوى قانونية في مجال العمل",
        RentDisputes: "نزاعات الإيجار",
        FamilyIssuesAndConditions: "قضايا الأسرة والظروف",
        Insurance: "التأمين",
        MaritimeDisputes: "نزاعات بحرية",
        BankingAndFinance: "البنوك والتمويل",
        TradingCompanies: "شركات التجارة",
        LitigationAndDisputeResolution: "التقاضي وحل النزاعات",
        PrecautionaryMeasures: "التدابير الاحترازية",
        ExecutiveActions: "إجراءات تنفيذية",
        RealEstateDisputes: "نزاعات العقارات",
        DebtCollection: "تحصيل الديون",

        // Legal Consultations
        Saleh_Al_Saleh_Legal_Consultations: "صالح الصالح للمحاماة والاستشارات القانونية",
        SalehAlSalehLegalConsultationsB1: "تقدم مكتب صالح الصالح للمحاماة والاستشارات القانونية النصائح القانونية من خلال اختيار متميز من المستشارين القانونيين. يتميز المكتب بأنه يواكب التطورات والتقنيات الحديثة. في هذا العصر المتسارع، يحتاج كل إنسان إلى معرفة حقوقه وواجباته والموقف القانوني تجاه أمر أو مشروع يقدم له، بسرعة ودقة كبيرتين. لذلك، أخذنا عناية بهذا الأمر، ولم نقتصر على الطرق التقليدية للتواصل بين المكتب وعملائه الراغبين في الحصول على استشارة قانونية بشأن موضوع معين. تقليديًا، يجب على أي عميل يرغب في الحصول على استشارة قانونية الذهاب إلى مكتب متخصص في هذا الشأن. ولكن في مكتب صالح الصالح للمحاماة والاستشارات القانونية، هناك اختلاف، حيث يمكنك إرسال استشارة قانونية لنا عبر البريد الإلكتروني، مشرحًا جميع التفاصيل المتعلقة بها. سيقوم أحد المستشارين المتخصصين بالمكتب بالتشاور فيها وإعطاء رأيهم القانوني، بدعم من نصوص القانون، ثم إرسالها لك عبر البريد الإلكتروني أو الفاكس، مع مراعاة سرية جميع البيانات والمعلومات الواردة في الاستشارة.",
        Saleh_Al_Saleh_Legal_ConsultationsB2: "نظرًا لأن استشارات القانون تلعب دورًا مهمًا وفعّالًا في توجيه وإرشاد العملاء قبل بدء الإجراءات القانونية، فإن المكتب مهتم بتقديم أفضل الخدمات للعملاء من خلال خدمة الاستشارات القانونية التي تقدمها فريق متخصص. يقدم هذا الفريق الاستشارات القانونية من خلال مستشارين مؤهلين، يتمتعون بخبرة ومعرفة متخصصة وعلمية، وذلك من خلال أحدث التقنيات سواء عبر الهاتف أو الإنترنت أو البريد الإلكتروني أو الفاكس أو الرسائل النصية. تتطلب خدمة الاستشارات القانونية تقديم النصائح بسرعة لتمكين المستجيب من اتخاذ القرارات بسرعة ودقة.",
        Saleh_Al_Saleh_Legal_ConsultationsB3: "نقدم استشارات باللغتين العربية والإنجليزية في جميع الجوانب القانونية التي تلبي احتياجات الأفراد والشركات في حياتهم اليومية أو في مسائل الاستثمار والأعمال، من خلال تقديم النصائح القانونية والإرشاد حول الإجراء المناسب لحماية مصالح العميل في كل مشكلة، لضمان فهم العميل لمشكلته وتداعياتها والإجراءات التي يجب اتخاذها لحل المشكلة وتفادي العواقب الجسيمة.",
        Saleh_Al_Saleh_Legal_ConsultationsB4: "يقدم المكتب استشارات قانونية سواء من خلال ترتيب موعد للاجتماع مع المحامي المختص والمستشار القانوني المسؤول في المكتب، أو من خلال المراسلات الإلكترونية أو تأكيد الهاتف. يقدم المكتب الاستشارات القانونية بشكل مكتوب وتغطيها الحقوق القانونية، والممارسات القضائية، وتفسيرات الفقهاء للإشارة إلى الارتباط القوي وضمان عميل المكتب من صحة الاستشارة القانونية ودعمها.",
        Saleh_Al_Saleh_Legal_ConsultationsB5: "المكتب مرتبط بعدة عقود استشارية سنوية مع الشركات التجارية والمؤسسات المالية التي ترغب في تلقي خدمات مستمرة طوال العام سواء كانت بموجب عقد ثابت برسم سنوي أو عقود بعدد ثابت أو غير محدد من الساعات، أو بموجب عقود تعتمد على ساعات العمل للمستشارين والباحثين القانونيين والمحامين لضمان تنوع الخيارات للعملاء.",
        Saleh_Al_Saleh_Legal_ConsultationsB6: "يُعتبر مكتب صالح الصالح مصدرًا موثوقًا؛ لأن الاستشارات القانونية التي يصدرها دائمًا ما ترافقها دليل ومصدر وبيان شامل يكفي لفهم العميل في البداية، لضمان أنه يطمئن إلى صحة الأسس التي صدرت عليها الاستشارة القانونية.",
        Lookingforlegaladvice: "هل تبحث عن استشارة قانونية؟ اتصل بنا، وسنبدأ تعاونًا مثمرًا.",

        // Preparation and drafting of legal documents
        Preparation_legal_documentsB: "هو المكتب الأشهر في التحضير وصياغة القانون في الإمارات حيث يتوفر لدينا فريق متخصص من المستشارين القانونيين والباحثين ذوي الخبرة في التحضير وصياغة النسخ القانونية والوثائق من مختلف الأشكال والأنواع. يقدم المكتب خدمات التحضير والصياغة باللغتين العربية والإنجليزية. وتشمل هذه جميع تنفيذ الشركات والمؤسسات ذات المسؤولية المحدودة، والمساهمات الخاصة والعامة، وريادة الأعمال والأفراد كأمثلة على مثل هذه الخدمات نحو صياغة وتحرير",
        Preparation_legal_documents1: "مذكرة التأسيس ومرفقاتها",
        Preparation_legal_documents2: "عقد واتفاقية الاندماج والاستحواذ",
        Preparation_legal_documents3: "عقود وكالة تجارية",
        Preparation_legal_documents4: "اتفاقيات التحكيم",
        Preparation_legal_documents5: "عقود واتفاقيات رعاية التجارة",
        Preparation_legal_documents6: "عقود التوقيف والوصية",
        Preparation_legal_documents7: "عقد الرهن",
        Preparation_legal_documents8: "عقود الاعتماد",
        Preparation_legal_documents9: "عقود العمل",
        Preparation_legal_documents10: "اتفاقيات المصالحة",
        Preparation_legal_documents11: "عقود التسويق",
        Preparation_legal_documents12: "اتفاقية إدارة المؤثرين والتأثيرات",
        Preparation_legal_documents13: "مذكرات التفاهم",
        Preparation_legal_documents14: "محاضر التصفية",
        Preparation_legal_documents15: "محاضر الجلسات العامة",
        Preparation_legal_documents16: "قسم الشهادة",
        Preparation_legal_documents17: "خطاب تفويض",
        Preparation_legal_documents18: "لوائح الانضباط الداخلي",
        Preparation_legal_documents19: "مذكرة الرأي القانوني",
        Preparation_legal_documents20: "الشروط والأحكام التنظيمية",
        Preparation_legal_documents21: "صياغة مذكرة الرأي والمشورة القانونية",
        Preparation_legal_documents22: "المراسلات القانونية",
        Preparation_legal_documents23: "صياغة الشكاوى والاقتراحات",
        Preparation_legal_documents24: "كتابة المراسلات القانونية",
        Preparation_legal_documents25: "عقود واتفاقيات الشراكة",
        Preparation_legal_documents26: "عقد المساهمين",
        Preparation_legal_documents27: "اتفاقيات سرية المعلومات",
        Preparation_legal_documents28: "عقود العلامات التجارية",
        Preparation_legal_documents29: "عقود حقوق العقارات",
        Preparation_legal_documents30: "عقود القروض",
        Preparation_legal_documents31: "عقود الإعفاء",
        Preparation_legal_documents32: "عقود الإيجار",
        Preparation_legal_documents33: "اتفاقيات التسوية",
        Preparation_legal_documents34: "عقود المقاولات",
        Preparation_legal_documents35: "عقود خدمات اللوجستيات",
        Preparation_legal_documents36: "الشروط والأحكام",
        Preparation_legal_documents37: "محاضر الجلسات العادية والغير عادية للجمعيات العامة لجميع الشركات",
        Preparation_legal_documents38: "محضر الإفلاس",
        Preparation_legal_documents39: "محاضر الاجتماعات",
        Preparation_legal_documents40: "وكالة عامة وخاصة",
        Preparation_legal_documents41: "اللوائح التنظيمية ولوائح الشركات",
        Preparation_legal_documents42: "مذكرات قانونية مختلفة",
        Preparation_legal_documents43: "مراجعة الوثائق القانونية",
        Preparation_legal_documents44: "المراجعة والتحليل القانوني للعقود والاتفاقيات",
        Preparation_legal_documents45: "تحذيرات قانونية",
        Preparation_legal_documents46: "صياغة التحذيرات القانونية",
        Preparation_legal_documents47: "كتابة مذكرات قانونية متنوعة",

        //Legal procedures, registration and legal credits
        LegalProcedures: "الإجراءات القانونية والتسجيل والاعتمادات القانونية",
        LegalProceduresB: "تقدم الإجراءات القانونية والتسجيل والاعتمادات القانونية في دولة الإمارات العربية المتحدة حيث لدينا فريق متخصص من المحامين والمساعدين القانونيين ذوي الخبرة في إجراء الإجراءات في مختلف المجالات ومختلف الأعمال الحكومية والخاصة بما في ذلك كافة أعمال التنفيذ القانوني. ومن أمثلة هذه الخدمات ما يلي:",
        LegalProceduresB1: "تسجيل الشركات والمؤسسات ذات المسؤولية المحدودة.",
        LegalProceduresB2: "تسجيل الشركات وريادة الأعمال",
        LegalProceduresB3: "العلامات التجارية",
        LegalProceduresB4: "الإفلاس",
        LegalProceduresB5: "تسجيل الشركات المساهمة الخاصة والعامة",
        LegalProceduresB6: "تسجيل الملكية الفكرية",
        LegalProceduresB7: "التصفية",
        LegalProceduresB8: "الإفلاس",
        LegalProceduresB9: "- التغيير: يشمل تغيير اسم التجارة، تغيير النشاط، تغيير إدارة العناصر، تغيير رأس المال، تغيير مدة الشركة أو أي شروط أخرى في العقد، تغيير واحد أو أكثر من أهداف شراكة الخدمة مع الشركاء، التغيير الكبير في الهيكل التنظيمي للخدمة أو الشركاء يؤثر على إدارة ووضع الشراكة، تغيير في هيكل الخدمات المشتركة بين الخدمة والشركاء، تغيير واحد أو أكثر من أهداف شراكة الخدمة مع الشركاء في هيكلها التنظيمي أو الشركاء يؤثر على إدارة ووضع الشراكة المبرمة، مراجعة العقد الشراكة الحالي وتقديم تقرير إلى الإدارة العليا يشير إلى نطاق تغيير الشراكة للتفاوض مع الشركاء بشأن الأهداف أو الخدمات التي يتعين تغييرها أو إضافتها إلى عقد الشراكة، التعديل والتغيير وفقًا لجودة الخدمات المقدمة من خلال الشركاء لتعديل أو تعديل التشريعات الحكومية والقوانين بمخالفة لاتفاقيات الشراكة المبرمة مسبقًا؛ إعداد عقد الشراكة المعدل بالتنسيق مع إدارة الشؤون القانونية، توقيع عقد الشراكة المعدل وتغطية الحدث بشكل إعلامي إذا كان التعديل ذو أهمية ويؤثر على العملاء؛ إنشاء مجموعة مشتركة لتنفيذ أحكام الاتفاقية المعدلة، ومجموعة مشتركة لتنفيذ أحكام الاتفاقية المعدلة.",
        LegalProceduresB10: "حصر الموروثات",
        LegalProceduresB11: "إثبات القدرة العقلية والأهلية",
        LegalProceduresB12: "المعلومات الشخصية لولي الأمر (مدعمة بجواز سفره)",
        LegalProceduresB13: "ما إذا كانت الوصية الأخرى تتعلق بملك الموصي ومكان تسجيله وما إذا كانت ترغب في إلغائها",
        LegalProceduresB14: "تفاصيل الحساب البنكي",
        LegalProceduresB15: "إثبات الدين",
        LegalProceduresB16: "العناصر اللازمة لكتابة الوصية",
        LegalProceduresB17: "شجرة العائلة (أفراد الأسرة ومن يعولون ولي أمرهم)",
        LegalProceduresB18: "تفاصيل الملكية مدعومة بنسخ من سندات الملكية",
        LegalProceduresB19: "تفاصيل أي عقار آخر",
        LegalProceduresB20: "يوفر المستشارون خدمات الشهادة من الجهات المختصة للوثائق الأجنبية الصادرة من الخارج، بالإضافة إلى خدمات الشهادة للوثائق المطلوبة من قبل الجهات الأجنبية في الخارج من خلال توثيق ختم المحامي المعتمد، وتشمل ذلك: توثيق نسخ جوازات السفر ونسخ فواتير الخدمات الصادرة عن السلطات المحلية وشهادات تأكيد العناوين للأفراد والشركات. تشمل خدمات التوثيق جميع الوثائق الأخرى مثل التراخيص التجارية وعقود الإنشاء والاتفاقيات والإعلانات وما إلى ذلك، التي قد تتطلب إنشاء شركات خارجية، أو فتح حسابات بنكية في الخارج، أو تقديم طلبات للهجرة والسفر.",
        LegalProceduresB21: "يقدم خدماته من خلال فحص صحة الوثائق والتحقق من صحة المعلومات الواردة في تلك البيانات ومصدرها، ثم يتم توثيقها وختمها بواسطة المحامي المعني، ويتلقى العميل نسخة معتمدة من الوثائق.",
        LegalProceduresB22: "تتضمن خدمات التوثيق التي تصدرها مكتب صالح الصالح للمحاماة والاستشارات القانونية تأكيد صحة توقيع الشخص أمام المحامي من خلال حضور الشخص الموقع على العقد وبعد التحقق من هويته وجواز سفره. يتم توقيع العقد أمام المحامي المختص الذي يوقع على صحة توقيع الشخص المذكور فيما يتعلق بمتطلبات بيع العقارات في المملكة المتحدة والدول الأجنبية.",
        LegalProceduresB23: "يمكن للجهات المستلمة للشهادات مراجعة مكتب صالح الصالح للمحاماة والاستشارات القانونية للتحقق من صحة توقيع المحامي، وسيقوم المكتب بتأكيد صحة الوثائق المصدرة لتلك السلطات.",

        //Legal Representation
        LegalRepresentation: "تمثيل قانوني",
        LegalRepresentationB: "العملاء الذين يقومون بالأعمال التجارية في الإمارات أو مع شركات إماراتية قد يجدون أنفسهم مسؤولين أمام المحاكم. لدينا خبراء قانونيون لديهم خبرة كبيرة في التقاضي التجاري في محاكم الإمارات، حيث يمثلون المدعين والمدعى عليهم. نحن نفهم الممارسات المعقدة لعملية التقاضي في المنطقة ويمكننا توفير تمثيل قانوني لكم في المنافسات العدائية الصعبة.",
        LegalRepresentationB1: "لدينا الخبرة والمعرفة في تسجيل ومتابعة جميع أنواع القضايا والدعاوى والمرافعات، والمعرفة العملية والعلمية في مختلف أنواع الإجراءات المدنية والتجارية والإدارية والتنفيذية، والقضايا العمالية والقانونية، والجنائية في جميع أنواع المحاكم من جميع الدرجات، الأولية - الاستئناف والمحكمة العليا الاتحادية (النقض) على مستوى الدولة.",
        LegalRepresentationB2: "يشمل ذلك: - تمثيل العملاء والمثول أمام جميع أنواع ومستويات المحاكم، بما في ذلك محكمة الدرجة الأولى، ومحكمة الاستئناف، ومحكمة النقض، والتنفيذ، والفحص أو التحقيق، وأمين العدل، والنائب العام، وهيئات قضائية أخرى، وخدمات الشرطة، وجميع الهيئات، والخدمات، والمراجع، والوزارات الرسمية الأخرى داخل وخارج دولة الإمارات العربية المتحدة.",
        LegalRepresentationB3: "حضور التحقيقات",
        LegalRepresentationB4: "تقديم الدعاوى، والمرافعة، ومتابعة الشكاوى، وتنفيذها، وما إلى ذلك",
        LegalRepresentationB5: "تقديم إخطار",

        //Research services
        Researchservices: "خدمات البحث",
        ResearchservicesB: "يقدم مكتب صالح الصالح للمحاماة والاستشارات القانونية خدمات البحث القانوني من خلال إعداد البحوث والدراسات القانونية المتخصصة في مختلف فروع القوانين والتشريعات من خلال قسم البحث العلمي والشرعي والفقهي. تلك الخدمات هي بلا شك أحد أهم المراجع المستخدمة في مجالات القانون والاستشارات. بالإضافة إلى ذلك، يقدم مكتب صالح الصالح للمحاماة والاستشارات القانونية خدمات التدريب القانوني من خلال تنظيم دورات تدريبية قانونية لموظفي الشركات الخاصة والعامة والمؤسسات. تلك الدورات تلتزم بمواصفات علمية عالية تتناسب مع جميع التخصصات القانونية وتستفيد من أعلى الكفاءات القانونية المتاحة.",
        //Penal cases
        PenalCases: "القضايا الجنائية",
        PenalCasesB: "يعمل مكتب صالح الصالح للمحاماة والاستشارات القانونية على تمثيل العملاء في الإجراءات الجنائية، سواء كانت مخالفات أو جنح أو جرائم أمام محاكم الدرجة الأولى، ومحاكم الاستئناف، ومحاكم النقض، وفقًا لأنواع هذه المحاكم الجنائية بين محاكم المرور والبلديات والنيابة العامة وأمن الدولة.",
        //Criminal Cases
        CriminalCases: "قضايا جنائية",
        CriminalCasesB: "يعمل مكتب صالح الصالح للمحاماة والاستشارات القانونية على تمثيل العملاء في العديد من الإجراءات الجنائية، سواء كانت مخالفات، جنح، أو جرائم، أمام محاكم الدرجة الأولى، ومحاكم الاستئناف، ومحاكم النقض لمختلف أنواع المحاكم الجنائية بين محاكم المرور، ومحاكم البلديات، ومحاكم النيابة العامة، ومحاكم أمن الدولة، والمحاكم العسكرية، بالإضافة إلى انتهاكات القوانين في أنواع مختلفة من الاتهامات ضد العملاء، بهدف تحقيق البراءة في مثل هذه القضايا والحصول على الصالحية.",
        CriminalCasesB1: "يتمثل دور مكتب صالح الصالح للمحاماة والاستشارات القانونية في تمثيل العملاء في الإجراءات الجنائية، والتي تتنوع بين جرائم المرور الخطيرة والمطالبات البلدية بشأن انتهاكات البناء، وعدم الامتثال لمعايير السلامة والمخالفات في المطاعم، بالإضافة إلى تمثيل العملاء أمام محاكم الهجرة، والبقاء في إقامة غير منتظمة وتقارير الهروب، وكذلك قضايا النيابة العامة مثل الإهانة، والقذف، والتشهير، والضرب، والاعتداء، والقتل الخطأ، وانتهاك الحكم، والاتجار وتهريب الممتلكات.",
        CriminalCasesB2: "يقوم مكتب صالح الصالح للمحاماة والاستشارات القانونية أيضًا بتمثيل العملاء في المخالفات والعقوبات الصادرة على أساس قوانين خاصة مثل جرائم الإنترنت بتنفيذ جرائم باستخدام وسائل تكنولوجيا المعلومات مثل الرسائل النصية وبرامج المحادثة وبرامج الاختراق، وكذلك انتهاك قوانين حماية الملكية الفكرية عن طريق وضع مواد محمية على الإنترنت، سواء للاستخدام الشخصي أو التجارة بها، بالإضافة إلى انتهاكات قوانين حماية حقوق المستهلك وجميع القوانين ذات الصلة بالانتهاكات.",
        CriminalCasesB3: "يعمل مكتب صالح الصالح للمحاماة والاستشارات القانونية على تحقيق براءة العملاء من خلال تقديم الدفاع أمام المحاكم، والحضور وتمثيل العميل، وتقديم الحجج التي تضمن تقديم الدفاع عن المتهمين والعملاء وضمان فهم القضاة للنزاع من خلال لغة واجادة المحامي الحاضر في النزاع، لضمان الحصول على أفضل النتائج وحماية مصالح العملاء.",
        //Cybercrime Lawsuit
        CybercrimeLawsuit: "قضية جرائم الإنترنت",
        CybercrimeLawsuitB: "تجارة التكنولوجيا العالمية. يتم اعتماد أحدث قوانين تكنولوجيا المعلومات لتمكينك من التفوق قانونيًا في هذا المجال من خلال مواكبة التحولات وفقًا للقانون الاتحادي رقم 1.",
        //Labor Lawsuit
        LaborLawsuit: "قضية عمالية",
        LaborLawsuitB: "يعمل مكتب صالح الصالح للمحاماة والاستشارات القانونية على تمثيل العديد من الموظفين وأصحاب العمل من المؤسسات والشركات والأفراد في القضايا المقدمة أمام محاكم العمل للمطالبة بتسوية الحسابات المستحقة بين الأطراف. يمثل مكتب صالح الصالح للمحاماة والاستشارات القانونية العديد من الموظفين وأصحاب العمل من المؤسسات والشركات والأفراد في الدعاوى المقدمة أمام محاكم العمل من أجل المطالبة بتسوية الحسابات المستحقة بين الأطراف. كما يمثل مكتب صالح الصالح للمحاماة والاستشارات القانونية الموظفين أو أصحاب العمل في تلك القضايا أمام محاكم الدرجة الأولى واستئنافية والنقض، ويتمتع بخبرة في مجال المحاسبة وتصفية مستحقات العمال بموجب عقود قضائية ثابتة وغير ثابتة ضد الشركات الخاصة أو الحكومية أو شبه الحكومية أمام كل من المحاكم البشرية والخاصة.",
        LaborLawsuitB1: "تمثل مكتب صالح الصالح للمحاماة والاستشارات القانونية الموظفين في المطالبة بحقوقهم العمالية من خلال مطالبة بالرواتب المتأخرة، والفصل التعسفي، ورصيد الإجازات المستحقة، ونهاية الخدمة، والإجازات التعسفية، والساعات الإضافية، والعمولات والنسب المستحقة، بالإضافة إلى طلب التعويض عن الأضرار المباشرة في حالة انتهاك صاحب العمل لالتزاماته، مثل تسليم المواد بشكل غير قانوني للموظفين، والمنافسة غير القانونية، وانتهاك سرية المعلومات، والأضرار التي تلحق بالمشغل نتيجة لخطأ فادح من الموظف، بالإضافة إلى إعفاء صاحب العمل من المطالبات بساعات العمل الإضافية للموظفين في مناصب الإدارة العليا.",
        LaborLawsuitB2: "تمتلك مكتب صالح الصالح للمحاماة والاستشارات القانونية خبرة عملية في تحقيق أفضل النتائج من خلال تقديم الحقائق والمبادئ القانونية التي تضمن للعملاء النتيجة المرجوة في حالات الفصل التعسفي والقضايا سواء بإجبار العامل على مغادرة العمل أو تقديم استقالته أو تقارير كاذبة عن الفرار، والتوقف التعسفي عن العمل وتمديد فترات الفصل والتوظيف بموجب عقود التكليف لضمان أن للعميل خبرات عملية أثناء تمثيله في النزاعات المطلوبة.",
        //Rent Disputes
        RentDisputes: "نزاعات الإيجار",
        RentDisputesB: "لا شك أن الإمارات من بين الدول التي تعتمد بشكل كبير على الاستثمار العقاري. يقوم مكتب صالح الصالح للمحاماة والاستشارات القانونية بإعداد وتقديم ومتابعة جميع الدعاوى الخاصة بالإيجار - الطرد - التعويض - استرجاع قيمة الإيجار - تعيين خبير لتقييم قيمة الإيجار للعقار وأية أضرار إذا وجدت. وقبل القاضي للأمور العاجلة إذا لزم الأمر في بعض الحالات وجميعها: الدرجة الأولى / الاستئناف / النقض.",
        //Family issues and conditions
        FamilyIssues: "قضايا الأسرة وأحوالها",
        FamilyIssuesB: "يعمل مكتب صالح الصالح للمحاماة والاستشارات القانونية على تمثيل العديد من الأفراد في النزاعات العائلية سواء كانوا مسلمين أو غير مسلمين. حيث يتمثل العمل في الدفاع والإجراءات أمام لجنة التوجيه العائلية في محاكم الأسرة داخل دولة الإمارات العربية المتحدة، وتتنوع القضايا من الطلاق وفقًا للقانون الدولة أو القانون الخاص بدول أجنبية أو قضايا المطالبة بالنفقة وحضانة الأطفال وفقًا للقانون الدولة.",
        FamilyIssuesB1: "النزاعات العائلية تمثل مجالًا واسعًا ومتشعبًا من الإجراءات والدعاوى أمام المحاكم. وهي تتضمن العديد من الإجراءات المؤقتة والسريعة لضمان حقوق الأطراف، بما في ذلك مطالبات الصيانة المؤقتة للزوجة التي يمتنع زوجها عن الإنفاق عليها، بما في ذلك النفقة للأطفال، تسليم الأطفال للأب أو الأم المتنازع عليهم، والرؤية المؤقتة، وتقديم النصائح والاستشارات للعملاء بشأن إجراءات المحاكمة خلال الجلسات، حيث يكون في بعض الأحيان حضور العميل الشخصي، والأطفال، وكذلك الإجراءات المتعلقة بالأطفال جزءًا كبيرًا من النزاعات.",
        FamilyIssuesB2: "يمثل مكتب صالح الصالح للمحاماة والاستشارات القانونية المتنازعين في الدعاوى العائلية من خلال سنوات من الخبرة في تحقيق أفضل النتائج للمتنازعين. يهتم المكتب بتحقيق هذه النتائج بشكل سريع، خاصة في حالات وجود الأطفال لحمايتهم لأنهم الطرف الضعيف في موضوع النزاع.",
        //Insurance
        Insurance: "تأمين",
        InsuranceB: "أدت النهضة التجارية والصناعية إلى نمو وازدهار قطاع التأمين وزيادة الوعي التأميني لدى الجميع. ونظرًا لتجاربنا النظرية والعملية في هذا المجال، يقدم مكتب صالح الصالح للمحاماة والاستشارات القانونية خدماته القانونية لقطاع التأمين في دولة الإمارات العربية المتحدة، سواء للشركات أو الأفراد.",
        InsuranceB1: "- تقديم الرأي القانوني والمشورة بشأن التغطية التأمينية للأفراد والشركات من مختلف الأنواع مثل حرائق العقارات، فقدان الممتلكات، حوادث السيارات، الإهمال المهني، الإصابات الشخصية.",
        InsuranceB2: "- تمثيل العملاء في جميع القضايا المتعلقة بالنزاعات التأمينية.",
        InsuranceB3: "- حل النزاعات التأمينية والتفاوض على الدفع مع شركات التأمين.",
        //Maritime disputes
        MaritimeDisputes: "النزاعات البحرية",
        MaritimeDisputesB: "يعد مكتب صالح آل صالح للمحاماة والاستشارات القانونية جزءًا من عدد قليل من المكاتب القانونية التي تتمتع بالمعرفة والأبحاث في مجال القانون البحري. ويحرص أعضاء مكتب صالح آل صالح للمحاماة والاستشارات القانونية على تقديم الأبحاث والدراسات اللازمة في هذا الشأن.",
        //Banking and Finance
        BankingFinance: "الخدمات المصرفية والمالية",
        BankingFinanceB: "تعد المؤسسات المصرفية والمالية في الدولة من أهم ركائز الاقتصاد الوطني. ولذلك فهم مهتمون للغاية بالاقتصاد. وقد أدرك مكتب صالح الصالح للمحاماة والاستشارات القانونية هذه الأهمية وعمل على تمثيل العديد من المؤسسات المصرفية واهتم بعمل هذه المؤسسات.",
        //Trading companies
        TradingCompanies: "الشركات التجارية",
        TradingCompaniesB: " يتمتع فريق مكتب الصالح للمحاماة والاستشارات القانونية بالخبرة والمتابعة المستمرة لجميع القوانين التي تحدد شكل الشركة وإجراءات تأسيس كل منها. يمكن لمحامينا ومستشارينا تقديم المشورة والتوجيه لجميع المستثمرين.",
        //Litigation and Dispute Resolution
        LitigationResolution: "التقاضي وحل النزاعات",
        LitigationResolutionB: "التقاضي هو الوسيلة الأكثر استخدامًا لتسوية المنازعات في دولة الإمارات العربية المتحدة، ولدى مكتب صالح آل صالح للمحاماة والاستشارات القانونية فريق غير مسبوق من المحامين المواطنين الموثوق بهم يدعمهم عدد من المستشارين القانونيين ذوي المعرفة والخبرة في العديد من الجوانب القانونية.",
        LitigationResolutionB1: "لدى صالح آل صالح للمحاماة والاستشارات القانونية فريق يتمتع بثقة غير مسبوقة من المحامين المواطنين يدعمه عدد من المستشارين القانونيين ذوي المعرفة والخبرة في العديد من الجوانب القانونية: المنازعات المدنية والتجارية والجنائية والعمالية والشركات. يقوم فريقنا بتمثيل جميع العملاء أمام مختلف المحاكم سواء في المحاكم الابتدائية / الاستئناف / النقض في الإمارات السبع.",
        //Precautionary measures
        PrecautionaryMeasures: "تدابير وقائية",
        PrecautionaryMeasuresB: "إصدار الأوامر بمصادرة الممتلكات والحسابات والأرصدة المصرفية لأي طرف ثالث، ومنع الأفراد من السفر وغيرها، وذلك لمنع الطرف الآخر من إخفاء الممتلكات أو مغادرة الدولة هرباً.",
        //Executive Actions
        ExecutiveActions: "تنفيذ الإجراءات",
        ExecutiveActionsB: "التمثيل القانوني في المحاكم يتم تمثيل محامي عملائنا أمام القضاة في المحاكم وفي جميع الإجراءات، بما في ذلك:",
        ExecutiveActionsB1: "رفع القضايا وإعداد الإنذارات والمرافعات والطعون والمخاطبات والمراسلات مع المحاكم والطرف الآخر خلال جميع مراحل التقاضي أمام المحكمة الابتدائية والاستئناف والنقض. تنفيذ الأحكام القضائية نعمل من خلال الإجراءات الإجبارية على تصفية ممتلكات الطرف الآخر وحساباته البنكية من أجل تنفيذ الأحكام القضائية.",
        // Real estate disputes
        RealEstateDisputes: "المنازعات العقارية",
        RealEstateDisputesB: "تعد المنازعات العقارية من أهم القضايا المتعلقة بالعقارات والتملك العقاري في دولة الإمارات العربية المتحدة. وتختلف هذه النزاعات حسب طبيعة العلاقة العقارية سواء كان الأمر يتعلق بملكية العقار أو بنائه أو إيجاره. وغيرها من النزاعات العقارية، وللتمكن من حل هذه النزاعات ومعالجتها بطريقة سليمة لتجنب الوقوع في أخطاء قد لا تحمد عقباها، يجب عليك الاستعانة بخدمات محامينا العقاري واستخدام خدماتنا المتقدمة لحل هذه النزاعات في بطريقة منهجية وتحليل المخاطر المرتبطة بها. وكذلك الاستمتاع بخدمات صياغة العقود العقارية، وكل ما يتعلق بالعقار.",
        RealEstateDisputesB1: "محامو المنازعات العقارية لصالح المحاماة والاستشارات القانونية يساعدونك على حل نزاعاتك العقارية بكل سهولة ويسر، لأنهم محترفون معترف بهم، حتى يتمكنوا من التعامل مع الأمور المتعلقة بنزاعاتك العقارية باحترافية مهما كانت درجة تعقيدها وحجم القضية، نظرا لخبرتهم الطويلة والمتراكمة في مجالات العقارات والمنازعات الناشئة عنها. ومن المعروف أن فريق العمل في مكتبنا متخصص في عدد من طرق حل النزاعات، بما في ذلك، على سبيل المثال لا الحصر، التحكيم والوساطة وغيرها من البدائل لحل النزاعات العقارية.",
        RealEstateDisputesB2: "وبما أن فريقنا من المحامين المتخصصين في العقارات يتمتعون بمهارات وخبرات طويلة وناجحة في هذا المجال، فإن طريقك إلى التقاضي، إذا لزم الأمر، يجب أن يكون من خلالهم؛ لأنهم الأكثر قدرة على متابعة هذه النزاعات أمام المحاكم.",
        //Debt Collection
        DebtCollection: "تحصيل الديون",
        DebtCollectionB: "لدى صالح آل صالح للمحاماة والاستشارات القانونية فريق من الخبراء ذوي الكفاءات العالية والخبرات المتنوعة في مجال تحصيل الديون والتسويات وإعادة الجدولة الودية مع العملاء في مختلف المجالات مثل الهندسة والبناء والمؤسسات المالية.",
        DebtCollectionB1: "صالح آل صالح للمحاماة والاستشارات القانونية في مجال تحصيل الديون وفق المعايير والقواعد والأنظمة المهنية وتحت مظلة شرعية متوافقة مع تعاليم الإسلام. يتمتع مكتب الأمم المتحدة المعني بالمخدرات والجريمة بالقدرة على تحقيق أكبر قدر من النجاح في استرداد الديون باستخدام العديد من الحلول التي قد تتغير تبعا للديون وحالة الديون وطبيعتها.",
        DebtCollectionB2: "لدى صالح آل صالح للمحاماة والاستشارات القانونية فريق من الخبراء ذوي الكفاءات العالية والخبرة المتنوعة في تحصيل الديون والتسويات وإعادة الجدولة الودية مع العملاء في مختلف المجالات مثل الهندسة والبناء والمؤسسات المالية والمصرفية والتأمين والشركات وغيرها.",
        DebtCollectionB3: "يتبع مكتب صالح ال صالح للمحاماة والاستشارات القانونية استراتيجية متخصصة لتحصيل الديون حيث يبدأ الفريق - أولا - بالتواصل مع المدين ومن ثم التحضير للقاء معه بحضور مستشار قانوني متخصص في تحصيل الديون والمنازعات ولديه المعرفة الكاملة للقوانين المعمول بها في هذا المجال، لغرض التفاوض على تسوية مبلغ الدين لصالح العميل",
        DebtCollectionB4: "إلا أن ذلك لا يمنع محامون صالح الصالح للمحاماة والاستشارات القانونية، إذا لزم الأمر، من اتخاذ الإجراءات القانونية اللازمة ضد المدين، وذلك من خلال رفع الجماعات القضائية ضده حسب الاقتضاء، سواء كانت جنائية أو مدنية.",

        // body
        Saleh_Al_Saleh: "صالح ال صالح للمحاماة و الاستشارات القانونية",
        body_index: "مكتب صالح الصالح للمحاماة والاستشارات القانونية هو واحد من أبرز مقدمي الخدمات القانونية في الإمارات العربية المتحدة من خلال الخبرة القانونية والطرق المهنية، مضمونًا رضى العملاء من المشاكل القانونية التي تواجهها. يتعامل المكتب في جميع أنواع القضايا المدنية والتجارية والجنائية ويقدم خدمات قانونية في مختلف المجالات وعلى جميع المستويات.",
        OurMission: "مهمتنا",
        OurVision: "رؤيتنا",
        OurMissionBody: "نحن نسعى لتقديم إرشاد قانوني استثنائي ودعم لعملائنا. نحن نسعى لتقديم حلول شخصية، مصممة لتلبية احتياجاتهم وظروفهم الفريدة. هدفنا ليس فقط توجيه العملاء عبر التعقيدات القانونية بفعالية، ولكن أيضًا تمكينهم بالمعرفة والثقة في شؤونهم القانونية. من خلال التزامنا بالنزاهة والاحترافية ورضا العملاء، نهدف إلى أن نكون شريكًا موثوقًا في تحقيق أهدافهم القانونية.",
        OurVisionBody: "نحن نسعى أن نكون الوجهة الرئيسية للخبرة القانونية والإرشاد، معترفين بالتزامنا الثابت بالتميز والابتكار ورضا العملاء. نطمح لوضع معايير جديدة في صناعة القانون من خلال تعزيز ثقافة التعاون والنزاهة والتفكير المستقبلي. من خلال التفاني للبقاء في صدارة الاتجاهات القانونية والتطورات، نهدف إلى تمكين الأفراد والشركات على حد سواء، مضمونين أنهم سيتخطون التحديات القانونية بثقة ووضوح.",

        OurServicesT: "خدماتنا",
        OurServicesB: "نحن نقدم مجموعة واسعة من الخدمات القانونية مصممة لتلبية احتياجات عملائنا.",
        OurServicesO: "هدفنا هو تقديم دعم قانوني موثوق وفعّال، يمكن العملاء من التنقل بثقة في المسائل القانونية المعقدة.",


        visiontolife: "تواصل معنا ودعنا نحقق رؤيتك على أرض الواقع.",
        english: 'English',
        arabic: 'عربي',
        language2: 'اللغة',
    },
};


const languageSelector = document.querySelector('select');
languageSelector.addEventListener('change', (event) => {
    setLanguage(event.target.value);
    localStorage.setItem('lang', event.target.value)

});
document.addEventListener('DOMContentLoaded', () => {

    setLanguage(localStorage.getItem("lang"));
})
const setLanguage = (language) => {
    const elements = document.querySelectorAll("[data-i18n]");
    elements.forEach((element) => {
        const translationsKey = element.getAttribute("data-i18n");
        element.textContent = translations[language][translationsKey];
    });
    // document.dir = language == "ar" ? "rtl" : "ltr";
    const ifrab = document.getElementById("ifarb");
    if(language == 'ar') {
    ifrab.setAttribute("dir","rtl");
    } else{
        ifrab.setAttribute("dir","ltr");
    }
};